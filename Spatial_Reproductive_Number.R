...
calc.lik_SP=function(x,y,lpdf_SP,CRS_TWD97=F){
  N=length(x)
  if(N!=length(y)) stop("different length")
  if(CRS_TWD97){
    z=as.matrix(lpdf_SP(dist(cbind(x,y))))
    z[lower.tri(z)]=0
    zz=Matrix(z)
    return(zz)
  }else{
    x <- as.numeric(x)*pi/180
    y <- as.numeric(y)*pi/180
    
    dxs <- rep(x, times=N) - rep(x, each=N)
    ys1 <- rep(y, times=N)
    ys2 <- rep(y, each=N)
    dys <- ys1 - ys2
    dss <- sin(dys/2)^2+ cos(ys1)*cos(ys2)*sin(dxs/2)^2
...
calc.Rj=function(t,x=NULL,y=NULL,lpdf_GI,lpdf_SP=NULL,CRS_TWD97=F,adj.sp=T,onlyPij=F){
  dd=data.frame(t=as.numeric(t))
  dd$count=1
  if(adj.sp){dd$x=as.numeric(x);dd$y=as.numeric(y)}
  dd=dd[order(dd$t),]
  
  lik_gi=calc.lik_GI(t=dd$t,lpdf_GI)
  
  if(adj.sp){
    lik_sp=calc.lik_SP(x=dd$x,y=dd$y,lpdf_SP,CRS_TWD97)
    lik_sp[lik_gi==0]=0
    lik_sp=drop0(lik_sp)
    lik_t=lik_gi+lik_sp
  }else{lik_t=lik_gi}
